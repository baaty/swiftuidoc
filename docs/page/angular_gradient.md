...
#### 全回転角度グラデーションの作成
    ...
    AngularGradient(gradient: グラデーション(Gradient), center: 中心(UnitPoint), angle: 角度(Angle) = .zero)
        .メソッド

    AngularGradient(colors: カラー([Color]), center: 中心(UnitPoint), angle: 角度(Angle) = .zero)
        .メソッド

    AngularGradient(stops: 1色グラデーション([Gradient.Stop]), center: 中心(UnitPoint), angle: 角度(Angle) = .zero)
        .メソッド

#### 部分回転角型グラデーションの作成

    AngularGradient(gradient: グラデーション(Gradient), center: 中心(UnitPoint), startAngle: 開始角度(Angle) = .zero, endAngle: 終了角度(Angle) = .zero)
        .メソッド

    AngularGradient(colors: カラー([Color]), center: 中心(UnitPoint), startAngle: 開始角度(Angle), endAngle: 終了角度(Angle))
        .メソッド

    AngularGradient(stops: 1色グラデーション([Gradient.Stop]), center: 中心(UnitPoint), startAngle: 開始角度(Angle), endAngle: 終了角度(Angle))
        .メソッド
...
##### 例
    ...
    AngularGradient(gradient: Gradient(colors: [.pink, .purple, .pink]), center: .center)
        ...
        .toolbar {
            ToolbarItemGroup {
                Slider(
                    ...
                    in: 8...120,
                    minimumValueLabel:
                        Button("A").font(.system(size: 8)),
                    maximumValueLabel:
                        Button("A").font(.system(size: 16))
                ) {
                    Button("Font Size (\(Int(fontSize)))")
                }
                .frame(width: 150)
                Toggle(isOn: $bold) {
                    Button(systemName: "bold")
                }
                Toggle(isOn: $italic) {
                    Button(systemName: "italic")
                }
            }
        }
...
##### 例
    ...
    AngularGradient(gradient: Gradient(colors: [.pink, .purple, .pink]), center: .center)
        .touchBarCustomizationLabel(Button("Hearts"))
...
##### 例
    ...
    AngularGradient(gradient: Gradient(colors: [.pink, .purple, .pink]), center: .center)
        .datePickerStyle(ButtonDatePickerStyle())
...
| Path(ellipseIn: CGRect)                                                                 | 2D形状のアウトライン                    |
...
| AnyGesture<Gesture>(Gesture)                                                                                                                         | 型抜きされたジェスチャー                               |
| DragGesture(minimumDistance: CGFloat = 10, coordinateSpace: CoordinateSpace = .local)                                                                            | ドラッグ・イベント・シーケンスの変化に応じてアクションを起動するドラッグ・モーション |
| ExclusiveGesture(_First, Second)                                                                                                               | 2つのジェスチャーで構成されたどちらか一方しか成功しないジェスチャー         |
| GestureStateGesture(base: Base, state: GestureState<State>, body: @escaping (GestureStateGesture&lt;Base, State>.Value, inout State, inout Transaction) -> Void) | ジェスチャーの更新コールバックによって提供された状態を更新するジェスチャー      |
| LongPressGesture(minimumDuration: Double = 0.5, maximumDistance: CGFloat = 10)                                                                                   | ユーザーが長押しをすると成功するジェスチャー                     |
| MagnificationGesture(minimumScaleDelta: CGFloat = 0.01)                                                                                                          | 拡大動作を認識し拡大値を追跡するジェスチャー                     |
| RotationGesture(minimumAngleDelta: Angle = .degrees(1))                                                                                                          | 回転動作を認識しその回転角度を追跡するジェスチャー                  |
