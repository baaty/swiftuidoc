...
##### 使い方
    ...
    .accessibilityScrollAction() { エッジ(Edge) in
        ユーザー補助のスクロールアクション
    }
...
##### 使い方
    ...
    .userActivity("宣伝する活動の種類", element: "要素") { 要素, 状態(NSUserActivity) in
        アクティビティを広告用に修正する機能(View)
    }

    .userActivity("宣伝する活動の種類", isActive: アクティビティの広告を回避するか(Bool)) { 状態(NSUserActivity) in
        アクティビティを広告用に修正する機能(View)
    }
...
##### 使い方
    ...
    .onContinueUserActivity("処理するアクティビティの種類") { 状態(NSUserActivity) in 
