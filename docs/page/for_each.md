...
識別されたデータの基礎となるコレクションから必要に応じてビューを計算する構造
...
| 名前       | 説明                   |
| -------- | -------------------- |
| onDelete | ダイナミックビューの削除アクションを設定 |
| onInsert | ダイナミックビューの挿入アクションを設定 |
| onMove   | ダイナミックビューの移動アクションを設定 |
...
ダイナミックビューの削除アクションを設定

##### 使い方

    .onDelete { 引数(IndexSet) in
        実行させたいアクション(Void)
    }

    .onDelete(perform: 実行させたいアクション)
...
ダイナミックビューの挿入アクションを設定

##### 使い方

    .onInsert(of: UTIタイプの配列([UTType])) { (引数1(Int), 引数2([NSItemProvider])) in
        実行させたいアクション(Void)
    }

    .onInsert(of: UTIタイプの配列([UTType]), perform: 実行させたいアクション)
...
ダイナミックビューの移動アクションを設定

##### 使い方

    .onMove { (引数1(IndexSet), 引数2(Int)) in
        実行させたいアクション(void)
    }

    .onMove(perform: 実行させたいアクション)
