...
使用できるかどうかを示すModifier

##### 使い方

    .handlesExternalEvents(matching: targetContentIdenfifierに含まれているかどうかチェックされるStringsのSet(Set<String>))

##### 例

    var hoge: Set = ["hoge"]
    Group {}
        .handlesExternalEvents(matching: hoge)
...
##### 使い方
    ...
    .onChange(of: 実行するかどうかを判断する際にチェックする値(Equatable)) {
        実行するアクション(Void)
    }

    .onChange(of: 実行するかどうかを判断する際にチェックする値(Equatable), perform: 実行するアクション)
...
##### 使い方
    ...
    .windowToolbarStyle(スタイル(WindowToolbarStyle))

##### 例

    Group {}
        .windowToolbarStyle(DefaultWindowStyle)
