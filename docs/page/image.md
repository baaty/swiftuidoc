...
##### 使い方
    ...
    .resizable(capInsets: リサイズしない画像の部分(EdgeInsets) = EdgeInsets(), resizingMode: リサイズする際のモード(Image.ResizingMode) = .stretch)
...
##### Image.ResizingModeの使い方

| 名前       | 説明                                   |
| -------- | ------------------------------------ |
| .stretch | スペースいっぱいに画像を拡大・縮小するモード               |
| .tile    | 画像を元のサイズで空きスペースを埋めるために必要な回数だけ繰り返すモード |
...
##### 使い方
    ...
    .renderingMode(レンダリングモード(Image.TemplateRenderingMode))

    .renderingMode()

##### Image.TemplateRenderingModeの使い方

| 名前        | 説明                                      |
| --------- | --------------------------------------- |
| .original | そのままレンダリングするモード                         |
| .template | 透明でないすべてのピクセルをフォアグラウンドカラーとしてレンダリングするモード |
...
##### 使い方
    ...
    .interpolation(品質レベル(Image.Interpolation))

##### Image.Interpolationの使い方

| 名前      | 説明                      |
| ------- | ----------------------- |
| .high   | 補間品質の高さを示す値             |
| .low    | 補間品質が低いことを示す値           |
| .medium | 低品質と高品質の中間のレベルの補間品質を示す値 |
| .none   | 画像データを補間しないことを示す値       |
...
##### 使い方
    ...
    .antialiased(アンチエイリアシングを許可するかどうか(Bool))
...
##### 例
    ...
    Image("テキスト")
        .accessibilityAction(named: Image("Custom Action 1"), customAction1)
...
##### 例
    ...
    Image("テキスト")
        .touchBarCustomizationLabel(Image("Hearts"))
...
縦横の比率を維持しながらどこかの部分がビューに収まるように伸縮
...
##### 例
    ...
    Image("テキスト")
        .actionSheet(isPresented: $isShowingSheet) {
            ActionSheet(
                title: Image("Permanently erase the items in the Trash?"),
                message: Image("You can't undo this action."),
                buttons:[
                    .destructive(Image("Empty Trash"),
...
##### 例
    ...
    Image("テキスト")
        .fullScreenCover(isPresented: $isPresenting,
                         onDismiss: didDismiss) {
            VStack {
                Image("A full-screen modal view.")
                    .font(.title)
...
##### 例
    ...
    Image("テキスト")
        .alert(isPresented: $isPresented) {
            Alert(title: Image("Order Complete"),
                  message: Image("Thank you for shopping with us."),
                  dismissButton: .default(Image("OK")))
        }
...
##### 使い方
    ...
    .previewConImage(プレビュー用のコンテキスト(PreviewContext) = nil)

##### 例

    Image("テキスト")
        .previewConImage(NogePreviewConImage(family: .systemSmall))
