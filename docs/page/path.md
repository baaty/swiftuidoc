...
| boundingRect   | CGRect              | すべてのパスセグメントを含む矩形           |
...
| addEllipse     | パスに楕円を追加                     |
| addLine        | 現在の点から指定した点までの直線セグメントを追加     |
| addLines       | パスに一連の接続された直線セグメントを追加        |
...
| applying       | パスのすべての点にトランスフォームを適用して作成したパス |
...
##### 使い方
    ...
    .addArc(center: 中心(CGPoint), radius: 半径(CGFloat), startAngle: 開始角度(Angle), endAngle: 終了角度(Angle), clockwise: 時計回りか(Bool), transform: トランスフォーム(CGAffineTransform) = .identity)

    .addArc(tangent1End: 1本目の線(CGPoint), tangent2End: 2本目の線(CGPoint), radius: 半径(CGFloat), transform: トランスフォーム(CGAffineTransform) = .identity)
...
##### 例
    ...
    var path = Path()
    path.addArc(center: CGPoint.zero, radius: 10, startAngle: .degrees(0), endAngle: .degrees(180), clockwise: true)
...
##### 例
    ...
    var path = Path()
    path.addCurve(to: CGPoint(), control1: CGPoint(x: 10, y: 10), control2: CGPoint(x: 20, y: 20))
...
##### 使い方
    ...
    .addEllipse(in: CGRect, transform: CGAffineTransform = .identity)
...
##### 例
    ...
    var path = Path()
    path.addEllipse(in: .zero)

#### addLine

##### 意味

現在の点から指定した点までの直線セグメントを追加
...
##### 使い方
    ...
    .addPath(Path, transform: CGAffineTransform = .identity)

##### 例

    var path = Path()
    var copy_path = Path()
    path.addPath(copy_path)

#### addQuadCurve

##### 意味

指定された終点と制御点を持つ二次ベジエ曲線をパスに追加

##### 使い方

    .addQuadCurve(to: CGPoint, control: CGPoint)
...
##### 例
    ...
    var path = Path()
    path.addQuadCurve(to: CGPoint(), control: CGPoint(x: 10, y: 10))
...
##### 使い方
    ...
    .addRect(CGRect, transform: CGAffineTransform = .identity)
...
##### 使い方
    ...
    .addRects([CGRect], transform: CGAffineTransform = .identity)
...
##### 例
    ...
    var path = Path()
    path.addRelativeArc(center: CGPoint.zero, radius: 10, startAngle: .degrees(0), delta: .degrees(45))
...
##### 使い方
    ...
    .addRoundedRect(in: CGRect, cornerSize: CGSize, style: RoundedCornerStyle = .circular, transform: CGAffineTransform = .identity)
...
##### 例
    ...
    var path = Path()
    path.addRoundedRect(in: .zero, cornerSize: CGSize())

#### applying

##### 意味

パスのすべての点にトランスフォームを適用して作成したパス
...
##### 使い方
    ...
    .contains(CGPoint, eoFill: Bool = false)
...
##### 使い方
    ...
    .trimmedPath(from: CGFloat, to: CGFloat)

##### 例

    var path = Path()
    path.trimmedPath(from: 0, to: 0)
...
バウンディングフレームを変更することなくこのシェイプをスケーリング
...
##### 使い方
    ...
    .stroke(色またはグラデーション(ShapeStyle), lineWidth: 幅(CGFloat) = 1)

    .stroke(色またはグラデーション(ShapeStyle), style: スタイル(StrokeStyle))
...
##### 使い方
    ...
    .fill(色またはグラデーション(ShapeStyle), style: スタイル(FillStyle) = FillStyle())
