...
##### 使い方
    ...
    .alignmentGuide(設定する構造体(HorizontalAlignment)) { 配置ガイド(ViewDimensions) in
        クロージャー(CGFloat)
    }

    .alignmentGuide(設定する構造体(HorizontalAlignment), computeValue: クロージャー)

    .alignmentGuide(設定する構造体(VerticalAlignment) { 配置ガイド(ViewDimensions) in
        クロージャー(CGFloat)
    }

    .alignmentGuide(設定する構造体(VerticalAlignment), computeValue: クロージャー)
