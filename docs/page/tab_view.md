...
対話可能なユーザーインターフェイス要素を使用して複数の子ビューを切り替えることができるビュー
...
#### Selectionの値に関連付けられたコンテンツを選択するインスタンスを作成
    ...
    TabView(selection: Selection(Binding<SelectionValue>)?) {
        コンテンツ
    }
        .メソッド

    TabView(selection: Selection(Binding<SelectionValue>)?, content: コンテンツ)
        .メソッド
